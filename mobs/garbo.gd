...
func on_collision_with_other_garbo(other: Garbo):
  ...
  if angle_to_other >= -135.0 and angle_to_other < -45.0:
    # # we used to do this here, now we use an Area2D for this logic
    # stack_garbo(other)
    pass
...
func set_stacked_object(object: StackedGarbo):
  if stacked_objects.size() > 0:
    ...
    push_error('you called set_stacked_object on me (a root garbo), but i already had a stacked object')
